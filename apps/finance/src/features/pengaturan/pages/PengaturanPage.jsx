/**
 * PengaturanPage.jsx — Edit tarif upah dasar (base upah).
...
import { DEFAULT_FINANCE_CONFIG, FINANCE_CONFIG_META } from "../utils";

export default function PengaturanPage() {
  const { config, loading } = useFinanceConfig();
  const saveConfig = useSaveFinanceConfig();
    ...
    try {
      await Promise.all(
        FINANCE_CONFIG_META.map(({ key }) =>
          saveConfig(key, DEFAULT_FINANCE_CONFIG[key])
        )
      );
