    ...
    <main className="min-h-screen bg-black text-white">
      <div className="md:grid md:grid-cols-[380px_1fr] md:min-h-screen">
        ...
        <aside className="md:sticky md:top-0 md:h-screen md:overflow-y-auto flex flex-col px-8 py-10 md:px-12 md:py-16 border-b border-white/5 md:border-b-0 md:border-r md:border-white/5">
          ...
          <p className="font-headline text-[#cab170] text-4xl leading-none">{product.kode}</p>
          <p className="mt-3 font-script text-white/65 text-2xl leading-tight">{product.nama}</p>

          <div className="w-10 h-px mt-5 mb-5 bg-[#cab170]/25" />

          {product.bahan && (
            <p className="font-editorial text-white/30 text-xs tracking-[0.15em] mb-6">{product.bahan}</p>
          )}

          {variants.length > 0 && (
            <div className="mb-8">
              <p className="font-editorial text-[10px] tracking-[0.3em] text-white/30 uppercase mb-3">Ukuran</p>
                  ...
                  <div key={v.size} className="flex items-center justify-between py-2.5">
                    ...
                    <div className="flex gap-4">
                      <span className="font-editorial text-xs text-white/45">
                        <span className="text-white/25 text-[10px] tracking-[0.1em] mr-1">LD</span>{v.ld} cm
                      </span>
                      <span className="font-editorial text-xs text-white/45">
                        <span className="text-white/25 text-[10px] tracking-[0.1em] mr-1">PB</span>{v.pb} cm
                      </span>
                    </div>
                  </div>
                ))}
