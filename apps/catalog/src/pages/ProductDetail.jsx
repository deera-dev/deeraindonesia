    ...
    <main className="min-h-screen bg-black text-white">
      <div className="md:grid md:grid-cols-[400px_1fr] md:min-h-screen">
        ...
        <aside className="md:sticky md:top-0 md:h-screen md:overflow-y-auto flex flex-col px-8 py-10 md:px-14 md:py-16 border-b border-white/5 md:border-b-0 md:border-r md:border-white/5">
          ...
          {product.bahan && (
            <p className="font-editorial text-white/35 text-base tracking-[0.1em] mb-7">{product.bahan}</p>
          )}

          {variants.length > 0 && (
            <div className="mb-8">
              <p className="font-editorial text-sm tracking-[0.3em] text-white/35 uppercase mb-4">Ukuran</p>
                  ...
                  <div key={v.size} className="flex items-center justify-between py-3.5">
                    ...
                    <div className="flex gap-5">
                      <span className="font-editorial text-base text-white/50">
                        <span className="text-white/25 text-xs tracking-[0.1em] mr-1.5">LD</span>{v.ld} cm
                      </span>
                      <span className="font-editorial text-base text-white/50">
                        <span className="text-white/25 text-xs tracking-[0.1em] mr-1.5">PB</span>{v.pb} cm
                      </span>
                    </div>
                  </div>
                ))}
        ...
        <div className="relative">
          {blurSrc && (
            <div className="hidden md:block fixed inset-0 z-0 pointer-events-none overflow-hidden" style={{left:"400px"}}>
            ...
            <div className="relative z-10 flex flex-col">
              {allPhotos.map((src, idx) => (
                <img key={idx} src={src}
                  ...
                  decoding="async" className="w-full h-auto block" />
              ))}
            </div>
          ) : (
            <div className="relative z-10 flex items-center justify-center h-64 md:h-screen">
              <p className="font-editorial text-white/20 text-base tracking-[0.3em]">FOTO BELUM TERSEDIA</p>
            </div>
          )}
        </div>
      </div>
