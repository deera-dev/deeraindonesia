...
export default function ImageSection({ mainImage, setMainImage, detailImages, setDetailImages, saving }) {
  ...
  function handleDetailAdd(e) {
    ...
    setDetailImages(prev => [...prev, ...files.map(f => ({ type:"file", file:f, preview:URL.createObjectURL(f) }))]);
    e.target.value = "";
  }
  function removeDetail(idx) { setDetailImages(prev => prev.filter((_,i) => i !== idx)); }
  function moveDetail(idx, dir) {
    setDetailImages(prev => {
      const arr = [...prev]; const ni = idx + dir;
      if (ni < 0 || ni >= arr.length) return arr;
      [arr[idx], arr[ni]] = [arr[ni], arr[idx]]; return arr;
    });
  }
          ...
          {detailImages.map((img, idx) => (
            <div key={idx} className="relative aspect-[3/4]">
              ...
              <div className="absolute flex justify-between gap-1 bottom-1 left-1 right-1">
                <button type="button" onClick={() => moveDetail(idx,-1)} disabled={idx===0||saving} className={btnSmall}>←</button>
                <button type="button" onClick={() => moveDetail(idx,1)} disabled={idx===detailImages.length-1||saving} className={btnSmall}>→</button>
              </div>
            </div>
          ))}
