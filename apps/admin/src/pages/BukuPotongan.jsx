/**
 * BukuPotongan.jsx — Perbandingan stok expected (buku potongan) vs aktual.
 *
 * Card per produk → components/buku/ProductBukuCard
 * Utilities       → components/buku/bukuUtils
 *
 * SQL tabel expected_stok (jika belum ada, lihat pesan error di halaman ini):
...
import { sortRows, kodeNum, rowKey } from "../components/buku/bukuUtils";
import ProductBukuCard from "../components/buku/ProductBukuCard";
...
export default function BukuPotongan() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (expRes.error?.code === "42P01") setTableError(true);
      ...
      setExpectedRows((prev) => {
        ...
        for (const row of upsertRows) {
          const idx = next.findIndex(
            (r) => r.kode === row.kode && r.size === row.size && r.warna === row.warna,
          );
          if (idx >= 0) next[idx] = { ...next[idx], expected_qty: row.expected_qty };
          else
            next.push({
              kode: row.kode,
              size: row.size,
    ...
    <main className="min-h-screen bg-skin-page text-skin-text pb-20">
      ...
      {tableError && (
        <div className="mx-4 mt-4 md:mx-8 px-4 py-3 bg-amber-50 border border-amber-300 text-sm text-amber-800 leading-relaxed">
          ...
          <p>Jalankan SQL di Supabase SQL Editor (lihat komentar di atas file ini).</p>
        </div>
      )}
