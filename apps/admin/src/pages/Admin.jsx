...
export default function Admin() {
  ...
  function loadStok() { fetchStokMap().then(setStokMap); }
  useEffect(() => { loadStok(); }, []);
  ...
  async function handleDelete(product) {
    ...
    const { error: delErr } = await supabase.from("products").delete().eq("kode", product.kode);
  ...
  return (
    <main className="min-h-screen bg-[#F9F7F4] text-[#1A1918]">

      {/* ── Header ── */}
      <header className="sticky top-0 z-30 bg-white border-b-2 border-[#E8E3DC] shadow-sm">
        <div className="flex items-center justify-between gap-3 px-4 py-4 md:px-8">
          <div className="min-w-0">
            <h1 className="font-headline text-[#CAB170] text-2xl leading-none md:text-3xl">DEERA</h1>
            <p className="mt-1 font-editorial text-xs tracking-[0.15em] text-[#9C9690] uppercase truncate">
          ...
          <div className="hidden md:flex items-center gap-2">
            <Link to="/admin/history"
              className="px-5 py-3 font-editorial text-sm tracking-[0.2em] uppercase text-[#6B6560] border-2 border-[#E8E3DC] hover:border-[#CAB170] hover:text-[#CAB170] transition">
              Riwayat
            </Link>
            ...
            <button onClick={handleLogout}
              className="px-5 py-3 font-editorial text-sm tracking-[0.2em] uppercase text-[#9C9690] border-2 border-[#E8E3DC] hover:text-red-600 hover:border-red-200 transition">
              Keluar
            </button>
          </div>
          ...
          <div className="flex items-center gap-2 md:hidden">
            ...
            <button onClick={() => setMenuOpen((v) => !v)}
              className="w-11 h-11 flex flex-col items-center justify-center gap-1.5 text-[#6B6560] border-2 border-[#E8E3DC] transition"
              aria-label="Menu">
              <span className={`block w-5 h-0.5 bg-current transition-all ${menuOpen ? "rotate-45 translate-y-[7px]" : ""}`} />
        ...
        {/* Mobile menu */}
        {menuOpen && (
          <div className="md:hidden border-t-2 border-[#E8E3DC] bg-white px-4 py-3 flex flex-col gap-1">
            <Link to="/admin/history" onClick={() => setMenuOpen(false)}
              className="py-3.5 font-editorial text-sm tracking-[0.2em] uppercase text-[#6B6560] border-b border-[#F0EBE3] hover:text-[#CAB170] transition">
              Riwayat
            </Link>
            <a href={`${CATALOG_URL}/catalog`} target="_blank" rel="noopener noreferrer"
      ...
      <div className="px-3 py-4 md:px-8 md:py-6">
        ...
        {loading && (
          <p className="font-editorial text-base text-[#9C9690] tracking-[0.2em] text-center py-20">Memuat produk...</p>
        )}
        {error && (
          ...
          <>
            ...
            {(products?.length ?? 0) > 0 && (
              <div className="mb-4">
                <input
                  ...
                  className="w-full bg-white border-2 border-[#E8E3DC] px-4 py-4 text-base text-[#1A1918] focus:outline-none focus:border-[#CAB170] transition font-editorial placeholder:text-[#C8C4C0]"
                />
                {q && (
                  <p className="mt-2 text-sm text-[#9C9690] font-editorial">
            ...
            {products?.length === 0 && (
              <div className="text-center py-20">
                <p className="font-editorial text-lg text-[#9C9690] tracking-[0.2em]">Belum ada produk</p>
                <button onClick={() => setEditing("new")}
            ...
            {sorted.length === 0 && products?.length > 0 && (
              <p className="text-center text-base text-[#C8C4C0] py-16 font-editorial">
      ...
      {editing && (
        <ProductForm
          ...
          onDelete={editing !== "new" ? () => { setEditing(null); handleDelete(editing); } : undefined}
          onSaved={() => {
            setEditing(null);
            invalidateProducts();
            loadStok();
