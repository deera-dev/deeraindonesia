...
import { buildNomor, fmtDate } from "../components/sampel/sampelUtils";
...
export default function ProduksiSampel() {
    ...
    try {
      if (editTarget) {
        ...
        logHistory({
          action: "sampel-edit",
          category: "produksi",
          kode: editTarget.nomor,
          nama: data.nama,
          snapshot: { nama: data.nama, tanggal: data.tanggal, foto: urls },
        }).catch(() => {});
      } else {
        // Create: data = [{ nama, tanggal }], urls = [[url,...], ...]
        // batch_id: optional — graceful fallback untuk browser lama / http
        let batchId = null;
        try { batchId = crypto.randomUUID(); } catch { /* ok */ }

        const inserts = data.map((entry, i) => ({
          nama: entry.nama,
        ...
        toast.success(`${inserts.length} sampel dibuat ✓`);
      }
      // Optimistic: langsung update UI, reload di background
      setShowForm(false);
      setEditTarget(null);
      load();
    } catch (err) {
  ...
  async function handleBatchSave(decs) {
    ...
    try {
      ...
      // Log history per keputusan
      toProcess.forEach(([id, dec]) => {
        const sampel = reviewBatch.find((s) => s.id === id);
        if (!sampel) return;
        logHistory({
      ...
      // Optimistic: langsung update status di list tanpa tunggu reload
      setSampels((prev) =>
        prev.map((s) => {
          const dec = decs[s.id];
          if (!dec || dec.choice === null) return s;
          ...
          return { ...s, status: "rejected", rejection_note: dec.alasan };
        }),
      );
      setReviewBatch(null);
      setDecisions({});
  ...
  async function handleDelete() {
    ...
    try {
      ...
      setSampels((prev) => prev.filter((s) => s.id !== id));
