...
function actionLabel(action) {
  ...
  if (action === "hapus")
    return { label: "Hapus", cls: "bg-red-100 text-red-700 border-red-200" };
  return {
    label: "Edit",
    cls: "bg-skin-gold text-[#CAB170] border-skin-bdr-gold",
  };
}
  ...
  return (
    <main className="min-h-screen bg-skin-page text-skin-text">
      <header className="sticky top-0 z-30 flex items-center justify-between gap-4 px-4 py-4 bg-skin-card border-b-2 border-skin-bdr shadow-sm md:px-8">
        <div>
          <h1 className="font-headline text-[#CAB170] text-2xl leading-none">
            DEERA
          </h1>
      ...
      <div className="px-4 py-6 md:px-8">
        {loading && (
          <p className="font-editorial text-base text-skin-text3 text-center py-16">
            Memuat riwayat...
          </p>
        )}
        {error && (
          <p className="font-editorial text-base text-red-600">
            {error.message}
          </p>
        )}
          ...
          <div className="flex flex-col divide-y divide-skin-bdr-lt">
            {history.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="py-5 flex flex-col gap-3 sm:flex-row sm:items-start sm:gap-6"
                >
                  <div className="min-w-[170px]">
                    ...
                    {item.user_name && (
                      <p className="mt-1 font-editorial text-sm text-skin-text2 font-medium">
                        {item.user_name}
                      </p>
                    )}
                  </div>
                  <span
                    className={`self-start px-3 py-1 font-editorial text-xs tracking-[0.15em] uppercase border rounded-sm ${cls}`}
                  >
                    {label}
                  </span>
                  <div className="flex-1">
                    <p className="font-headline text-[#CAB170] text-xl leading-none">
                      {item.kode}
                    </p>
                    <p className="mt-1 font-editorial text-base text-skin-text2">
                      {item.nama || snap.nama || "-"}
                    </p>
                    {snap && Object.keys(snap).length > 0 && (
                      <div className="mt-2 flex flex-wrap gap-x-5 gap-y-1">
                        {snap.bahan && (
                          <span className="font-editorial text-sm text-skin-text3">
                            Bahan: {snap.bahan}
                          </span>
                        )}
                        {snap.hpp != null && (
                        ...
                        {Array.isArray(snap.variants) &&
                          snap.variants.length > 0 && (
                            <span className="font-editorial text-sm text-skin-text3">
                              Ukuran:{" "}
                              {snap.variants.map((v) => v.size).join(" · ")}
                            </span>
                          )}
                      </div>
                    )}
