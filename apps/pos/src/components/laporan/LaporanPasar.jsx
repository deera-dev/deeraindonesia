...
function calcRingkasan(rows, loc) {
  ...
  const keuntungan = lokasiFilt.reduce(
    (s, t) => s + (t.items ?? []).reduce((ss, item) => ss + itemProfit(item), 0), 0,
  );
...
export default function LaporanPasar({ sales }) {
  ...
  // Load history dari Supabase (30 hari terakhir, lokasi pasar saja)
                ...
                <div className="border-t border-skin-bdr px-4 pb-4 pt-3 space-y-3">
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <StatCard
                      ...
                      color="green"
                    />
                    <StatCard label="Transaksi" value={String(r.lokasiFilt.length)} color="neutral" />
                    <StatCard
                      label="Retur"
                  ...
                  {r.topProds.length > 0 && (
                    <div className="border border-skin-bdr">
                      ...
                      {r.topProds.map((p, i) => (
                        <div key={p.kode} className="flex items-center justify-between px-3 py-2 border-b border-skin-bdr-lt last:border-0">
          ...
          {["hari-ini", "history"].map((m) => (
            <button key={m} type="button" onClick={() => setMode(m)}
              className={`flex-1 py-2 font-editorial text-xs tracking-[0.12em] uppercase border transition ${
                mode === m ? "border-[#CAB170] text-[#CAB170] bg-skin-gold" : "border-skin-bdr text-skin-text3"
              }`}>
  ...
  async function handleDownload() {
    ...
    try {
      const dataUrl = await toPng(reportRef.current, {
        quality: 1, pixelRatio: 2.5, backgroundColor: "#ffffff",
      });
