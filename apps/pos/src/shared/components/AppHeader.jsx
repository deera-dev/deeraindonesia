 ...
 */
import { signOut, displayName } from "@deera/shared/features/auth/hooks";
...
export default function AppHeader({
  ...
  return (
    <header className="bg-skin-card border-b border-skin-bdr shadow-sm z-30 flex-shrink-0 overflow-x-hidden">
      {/* ── Baris utama ── */}
      <div className="flex items-center justify-between px-4 py-3 md:px-6">
        {/* Kiri: logo + status */}
        <div className="flex items-center gap-3 min-w-0">
          <h1 className="text-3xl text-[#CAB170] leading-none flex-shrink-0 font-headline">
            DEERA
          </h1>
          ...
          >
            ...
            {isOnline && (
              <span
                className={`text-base leading-none ml-0.5 ${syncing ? "animate-spin inline-block" : ""}`}
              >
                ↻
              </span>
            )}
            {syncLabel && (
              <span className="hidden sm:inline text-xs opacity-70 ml-0.5">· {syncLabel}</span>
            )}
          </button>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <span className="hidden md:inline text-skin-text2 mr-1">{displayName(user)} ·</span>
            Keluar
          </button>
        </div>
      </div>
      ...
      {failedCount > 0 && !syncError && (
        <div className="bg-amber-50 border-t border-amber-100 px-4 py-2 flex items-center justify-between">
          <p className="text-sm text-amber-800">⏳ {failedCount} transaksi belum tersync</p>
